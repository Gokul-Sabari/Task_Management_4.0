...
export const getTaskById = async (req: Request, res: Response) => {
    try {
        ...
        const validation = validateWithZod<{ id: number }>(taskIdSchema, { id });
...
export const getTasksByTaskGroup = async (req: Request, res: Response) => {
    try {
        const taskGroupIdParam = getStringParam(req.params.taskGroupId);
        
        if (!taskGroupIdParam || isNaN(parseInt(taskGroupIdParam))) {
            return res.status(400).json({
                success: false,
                message: 'Valid task group ID is required'
            });
        ...
        const taskGroupId = parseInt(taskGroupIdParam);

        const tasks = await Task.findAll({
            where: { Task_Type_Id: taskGroupId },
            order: [['Task_Name', 'ASC']]
        });
...
export const updateTask = async (req: Request, res: Response) => {
    try {
        ...
        const idValidation = validateWithZod<{ id: number }>(taskIdSchema, { id });
