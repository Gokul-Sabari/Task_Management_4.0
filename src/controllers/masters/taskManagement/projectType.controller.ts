...
// Helper function to safely parse ID parameters
...
// Helper function to get string value from param
...
export const getProjectById = async (req: Request, res: Response) => {
    try {
        ...
        const validation = validateWithZod<{ id: number }>(projectIdSchema, { id });
...
export const getProjectsByCompany = async (req: Request, res: Response) => {
    try {
        const companyIdParam = getStringParam(req.params.companyId);
        
        if (!companyIdParam || isNaN(parseInt(companyIdParam))) {
            return res.status(400).json({
                success: false,
                message: 'Valid company ID is required'
            });
...
export const getProjectsByProjectHead = async (req: Request, res: Response) => {
    try {
        const projectHeadIdParam = getStringParam(req.params.projectHeadId);
        
        if (!projectHeadIdParam || isNaN(parseInt(projectHeadIdParam))) {
            return res.status(400).json({
                success: false,
                message: 'Valid project head ID is required'
            });
        ...
        const projectHeadId = parseInt(projectHeadIdParam);

        const projects = await Project.findAll({
            where: { 
                Project_Head: projectHeadId,
                IsActive: 1 
            },
            order: [['Project_Name', 'ASC']]
        });
...
export const getProjectsByStatus = async (req: Request, res: Response) => {
    try {
        const statusParam = getStringParam(req.params.status);
        ...
        if (projects.length === 0) {
            return res.status(200).json({
                success: true,
                message: `No projects found with status ${statusNum}`,
                data: []
            });
        }
...
export const updateProject = async (req: Request, res: Response) => {
    try {
        ...
        const idValidation = validateWithZod<{ id: number }>(projectIdSchema, { id });
